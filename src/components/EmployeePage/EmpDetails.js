...
export default function EmpDetails() {
  ...
  const getDetails = useCallback(async () => {
    ...
    try {
      ...
      if (u) {
        ...
        if (u.attendencepermission)
          setAttendencePermission(u.attendencepermission);
        ...
        if (u.inventorypermission)
          setInventoryPermission(u.inventorypermission);
  ...
  useEffect(() => {
    ...
    const fetchMasters = async () => {
      try {
        ...
        const desRes = await API.get(
          `/master/designations?partnerId=${partnerId}`,
        );
        ...
        <div className="row g-3">
          {Object.keys(state).map((key) => (
            <div key={key} className="col-12 col-md-6 col-lg-4 col-xl-3">
              <label
                className={`permission-tile d-flex align-items-center justify-content-between gap-2 p-3 rounded-3 border transition-all ${state[key] ? "bg-white border-primary-subtle shadow-sm" : "bg-white border-light opacity-75"}`}
              >
                <div className="d-flex align-items-center gap-2 overflow-hidden">
                  ...
                  />
                  <span
                    className={`small fw-medium text-capitalize text-truncate ${state[key] ? "text-dark" : "text-muted"}`}
                  >
  ...
  return (
    <div
      style={{ backgroundColor: "#cecef4" }}
      className="employee-details-page min-vh-100"
    >
      <ToastContainer position="bottom-center" />

      {/* --- Responsive Header --- */}
      <header
        className="bg-white shadow-sm border-bottom sticky-top rounded-3"
        style={{ zIndex: 1020 }}
      >
        <div className="container-fluid px-3 px-md-4 py-3">
          <div className="d-flex flex-row justify-content-between align-items-center">
            <div className="d-flex align-items-center gap-3">
              ...
              <div className="overflow-hidden">
                <h5 className="mb-0 fw-bold text-dark text-truncate">
                  {empData.FULLNAME || "Employee Detail"}
                </h5>
                <div className="d-flex align-items-center gap-2 text-muted small">
                  <span
                    className={`badge ${empData.status === "active" ? "bg-success" : "bg-danger"} rounded-pill`}
                    style={{ fontSize: "10px" }}
                  >
                    {empData.status}
                  </span>
                  ...
                  <span className="d-flex align-items-center gap-1">
                    <Briefcase size={12} /> {empData.DESIGNATION || "General"}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="d-flex gap-4 mt-3 mb-n3 nav-tabs-modern">
              ...
              <button
                className={`nav-tab-item ${activeTab === "permissions" ? "active" : ""}`}
            ...
            <div className="d-flex gap-2">
              {!isEditing ? (
                <button
                  className="btn btn-primary rounded-3 px-3 px-md-4 d-flex align-items-center gap-2 shadow-sm"
                ...
                <div className="d-flex gap-2">
                  <button
                    className="btn btn-success rounded-3 px-3 px-md-4 d-flex align-items-center gap-2 shadow-sm"
                    onClick={handleSave}
                  >
                    <Save size={18} />{" "}
                  ...
                  <button
                    className="btn btn-outline-secondary rounded-3 px-3 shadow-sm"
      ...
      <main className="container-fluid mt-3 mt-lg-4 px-3 px-md-4">
        {isLoading ? (
            ...
            {activeTab === "profile" ? (
              <>
                ...
                <div className="col-12 col-lg-8">
                  <section className="card border-0 shadow-sm rounded-4 p-4 mb-2">
                      ...
                      <div className="col-md-6">
                        ...
                        <div className="input-group input-group-merge">
                          ...
                          <input
                            ...
                            readOnly={!isEditing}
                            className={`form-control border-0 bg-light ${isEditing ? "border-primary-subtle" : ""}`}
                          />
                        </div>
                      </div>
                ...
                <div className="col-12 col-lg-4">
                  <div
                    className="card border-0 shadow-sm rounded-4 p-4 mb-4 bg-primary text-white"
                    style={{
                      background:
                  ...
                  >
                    ...
                    <div className="mb-3 border-bottom border-white border-opacity-25 pb-3">
                      <label className="small opacity-75 d-block mb-1">
                        Bank Name
                      </label>
                      <input
                        name="BANKNAME"
                        ...
                        onChange={handleChange}
                        readOnly={!isEditing}
                        className="form-control form-control-sm bg-white bg-opacity-10 border-0 text-white placeholder-white-50"
                      />
                    </div>
              ...
              <div className="col-12">
                <div className="mb-2 p-3 bg-primary bg-opacity-10 border border-primary-subtle rounded-3 d-flex align-items-start">
                  <AlertCircle className="text-primary mt-1" size={20} />
                  <div>
                    <p className="small mb-0 fw-medium text-primary">
                      Granular Access Control
                    </p>
                    <p className="small mb-0 text-muted">
                      Use the master switches to grant all permissions in a
                      module or select individual actions below.
                    </p>
                  </div>
                </div>
                <PermissionSection
                  ...
                  icon={User}
                  state={customerpermission}
                  setter={setCustomerPermission}
                />
                <PermissionSection
                  title="Master Controls"
                  icon={ShieldCheck}
                  state={masterpermission}
                  setter={setMasterPermission}
                />
                <PermissionSection
                  title="Lead Management"
                  icon={ChevronRight}
                  state={leadpermission}
                  setter={setLeadPermission}
                />
                <PermissionSection
                  title="Payment & Collections"
                  icon={CreditCard}
                  state={paymentpermission}
                  setter={setPaymentPermission}
                />
                <PermissionSection
                  title="Network Management"
                  icon={MapPin}
                  state={networkpermission}
                  setter={setNetworkPermission}
                />
                <PermissionSection
                  title="Inventory Control"
                  icon={Briefcase}
                  state={inventorypermission}
                  setter={setInventoryPermission}
                />
                <PermissionSection
                  title="Attendance Rules"
                  icon={Clock}
                  state={attendencepermission}
                  setter={setAttendencePermission}
                />
                <PermissionSection
                  title="HR & Employee"
                  icon={User}
                  state={employeepermission}
                  setter={setEmployeePermission}
                />
              </div>
            )}
          </div>
