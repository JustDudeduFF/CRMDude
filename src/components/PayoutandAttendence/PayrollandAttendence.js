...
export default function PayrollandAttendence() {
  ...
  const [showDeductionDetailsModal, setShowDeductionDetailsModal] =
    useState(false);
  ...
  const generateDays = () => {
    ...
    for (let i = 1; i <= daysInMonth(selectedYear, selectedMonth); i++) {
      ...
      days.push(
        <th
          key={i}
          className="text-center p-1 sticky-header-cell"
          style={{ minWidth: "45px" }}
        >
          ...
          <div
            style={{
              fontSize: "0.7em",
              color: dayName === "Sun" ? "#ef4444" : "#64748b",
            }}
          >
            {dayName}
          </div>
  ...
  return (
    ...
    >
        ...
        <header className="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
          <div>
            ...
            <p className="text-muted small mb-0">
              Manage employee attendance, allowances, and monthly payroll
            </p>
          </div>
                ...
                <tbody>
                  {payoutArray.map((item, index) => (
                    <tr key={item.employeeId}>
                      ...
                      >
                        <MinusCircle size={14} className="me-1" />₹
                        {item.netSalary.totalDeduction}
                      </td>
            ...
            <div className="table-responsive" style={{ maxHeight: "70vh" }}>
              <Table bordered hover size="sm" className="mb-0 text-nowrap">
                <thead className="bg-light sticky-top" style={{ zIndex: 11 }}>
                  <tr>
                    <th
                      className="sticky-left bg-light z-3"
                      ...
                      {Array.from(
                        ...
                        (_, i) => {
                          ...
                          return (
                            <td
                              key={day}
                              className={`text-center fw-bold p-2 ${statusClass}`}
      ...
      <Modal
        ...
        onHide={() => setShowAllowanceModal(false)}
        centered
        className="rounded-4"
      >
        ...
        <Modal.Body className="pt-3">
          <div className="mb-3">
            ...
            <select
              value={allowanceInput.type}
              onChange={(e) =>
                setAllowanceInput((prev) => ({ ...prev, type: e.target.value }))
              }
      ...
      <Modal
        show={showDeductionDetailsModal}
        onHide={() => setShowDeductionDetailsModal(false)}
        centered
      >
        <Modal.Header closeButton className="border-0 pb-0">
        ...
        <Modal.Body className="pt-3">
          <div className="mb-3">
            ...
            <select
              ...
              onChange={(e) =>
                setDeductionInput((prev) => ({ ...prev, type: e.target.value }))
              }
              className="form-select bg-light border-0 py-2"
            >
              ...
              <Card className="border-0 shadow-sm h-100">
                <Card.Body>
                  ...
                  <div className="d-flex justify-content-between text-muted small">
                    ...
                    <span>
                      ₹{selectedUserDetails?.netSalary.gross?.toFixed(2)}
                    </span>
                  </div>
                </Card.Body>
              </Card>
            ...
            <div className="col-md-6">
              <Card className="border-0 shadow-sm h-100">
                <Card.Body>
                  ...
                  {(selectedUserDetails?.allowances || []).length === 0 && (
                    <div className="text-muted">No allowances</div>
                  )}

                  {selectedUserDetails?.allowances?.map((item, index) => (
                    <div
              ...
              <Card className="border-0 shadow-sm h-100">
                <Card.Body>
                  <h6 className="fw-bold mb-3 border-bottom pb-2 text-danger">
                    Deducitons & Penalities
                  </h6>

                  {(selectedUserDetails?.deductions || []).length === 0 && (
                    <div className="text-muted">No deductions</div>
                  )}

                  {selectedUserDetails?.deductions?.map((item, index) => (
                    <div
                      className="d-flex justify-content-between mb-2"
                      key={index}
                    >
              ...
              <div style={{background:"linear-gradient(135deg, #667eea 0%, #764ba2 100%)"}} className="text-white p-3 rounded-4 d-flex justify-content-between align-items-center">
                <div>
                  ...
                  <h3 className="mb-0 fw-bold">
                    ₹{selectedUserDetails?.netSalary.net?.toFixed(2) || "0.00"}
                  </h3>
                </div>
                ...
                {advanceDetails.length > 0 ? (
                  <div className="list-group list-group-flush">
                    {advanceDetails.map((adv, i) => (
                      <div
                        className="list-group-item d-flex justify-content-between align-items-center px-0 bg-transparent border-light"
                        key={i}
                      >
                        <div>
                          <div className="fw-bold">
                            {adv.title || "Adjustment"}
                          </div>
