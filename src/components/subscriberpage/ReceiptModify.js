  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const handleDownloadInvoice = async() => {
        ...
        try{
          const response = await axios.post('http://api.justdude.in:5000/send-invoice', mailData);
          if(response.ok){
    ...
    const sendWhatsapp = async () => {
      ...
      const response = await axios.post(`http://api.justdude.in:5000/send-message?number=91${contact}&message=${encodedMessage}`);
