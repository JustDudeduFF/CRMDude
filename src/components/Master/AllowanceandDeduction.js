...
export default function AllowanceAndDeduction() {
  ...
  const [arrayallowanceAnddeduciton, setArrayallowanceAnddeduciton] = useState(
    [],
  );

  const fetchAllowanceandDeduction = async () => {
    try {
      ...
      setArrayallowanceAnddeduciton(response.data);
    } catch (e) {
      console.log(e);
    ...
    <div className="dc-container">
      ...
      <div className="dc-header-card">
        <div className="dc-title-section">
          ...
          <div>
            <h5 className="dc-main-title">Allowance & Deduction Particulars</h5>
            <p className="dc-subtitle">
              Manage allowance and deduction categories
            </p>
          </div>
        </div>
        ...
        <button
          onClick={() =>
            hasPermission("ADD_ALLOWANCE_DEDUCTION_CONCERN")
              ? setShowModal(true)
      ...
      <DebitCreditNoteModal
        ...
        notshow={() => {
          setShowModal(false);
          fetchAllowanceandDeduction(); // Refresh list after adding
        }}
      />
      ...
      <div className="dc-table-wrapper">
        <div className="dc-table-responsive">
          <table className="dc-custom-table">
            ...
            <tbody>
              {arrayallowanceAnddeduciton.length > 0 ? (
                arrayallowanceAnddeduciton.map(
                  ({ title, type, createdAt }, index) => (
                    <tr key={index}>
                      ...
                      <td className="dc-name-cell">
                        ...
                        <span className="dc-particular-link">{title}</span>
                      </td>
                      <td className="dc-name-cell">
                        <span className="mobile-label">Type:</span>
                        <span className="dc-particular-link">{type}</span>
                      </td>
