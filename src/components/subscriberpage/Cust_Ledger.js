  ...
  const fetchLedger = async () => {
    ...
    try {
      const response = await axios.get(
        `${api2}/subscriber/ledger?id=${userid}`
      );
        ...
        <div className="cust-ledger-header">
          ...
          <div className="cust-ledger-actions">
            <img
              src={Excel_Icon}
              alt="Download Excel"
              className="cust-ledger-download-icon"
            />
            <img
              src={PDF_Icon}
              alt="Download PDF"
          ...
          <table className="cust-ledger-table">
            ...
            <tbody>
              {isLoading ? (
                ...
                [...arrayledger] // clone so original isn't mutated
                  .sort((a, b) => new Date(a.date) - new Date(b.date)) // oldest first, newest last
                  .map(
                    (
                      { type, date, particular, debitamount, creditamount },
                      index
                    ) => {
                      ...
                      return (
                        <tr
                          className={
                            debitamount > 0 ? "table-danger" : "table-success"
                          }
                          key={index}
                        >
                          <td>{index + 1}</td>
                          ...
                          <td
                            className={
                              debitamount > 0 ? "cust-ledger-debit" : ""
                            }
                          >
                          ...
                          <td
                            className={
                              creditamount > 0 ? "cust-ledger-credit" : ""
                            }
                          >
                            {creditamount.toFixed(2)}
                          </td>
                          <td className="cust-ledger-balance">
                ...
                <tr>
                  <td colSpan="8" className="cust-ledger-no-data">
