    ...
    const handleSavePlan = async (username, expireDate, planAmount, planName, mobile, fullName) => {
        ...
        if (selectedPlanObj) {
            ...
            const sendMessage = async (mobile, planName, fullName, expireDate, planAmount, date) => {
                ...
                const response = await axios.post(`http://api.justdude.in:5000/send-message?number=91${mobile}&message=${encodedMessage}`);
                const responsemail = await axios.post('http://api.justdude.in:5000/sendmail', {
        ...
        if(heading.split(" ")[0] === "Expiring"){
            if(hasPermission("MSG_EXPIRING")){
                ...
                filteredArray.forEach(async(data) => {
                    ...
                    await axios.post(`http://api.justdude.in:5000/send-message?number=91${mobile}&message=${encoedeMessage}`);
                });
            }else{
